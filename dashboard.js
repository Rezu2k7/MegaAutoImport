(function() {
    ...
    async function loadCars() {
        try {
            ...
            myCars.forEach(car => {
                ...
                const previewImg = car.images?.[0] ? `http://localhost:5000/uploads/${car.images[0]}` : '';
    ...
    window.openCarMenu = (car) => {
        ...
        if (car.images && car.images.length > 0) {
            car.images.forEach(img => { galleryHtml += `<img src="http://localhost:5000/uploads/${img}">`; });
        } else {
        ...
        if (role === 'admin') {
            ...
            if (car.documents && car.documents.length > 0) {
                car.documents.forEach(doc => { docsHtml += `<li style="margin-bottom:5px;"><a href="http://localhost:5000/uploads/${doc.filename}" target="_blank" style="color:#ffcc00; text-decoration:none;">📄 ${doc.originalName}</a></li>`; });
            } else {
